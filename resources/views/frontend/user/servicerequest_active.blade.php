                ...
                <tr>
                  ...
                  <td>
                    ...
                    <div class="trtouter">
                        <div class="trtleft">
                            <div class="acebtngroup">
                              <a class="acebtn" href="javascript:void(0);" data-orderId="{{ $order->id }}" data-toggle="modal" data-target="#BidModal{{ $order->id }}">Make an offer</a>
                              <!--<a class="acebtn" href="javascript:void(0);">Get it Now</a>-->
                            </div>
                        </div>
                        <div class="trtright">
                        <!--<p>Want it now?  &#8377; 8,000</p>-->
                        @if($order->low_price) <p>Recent Bid &#8377; {{ $order->low_price }}</p> @endif
                        
                        <div class="acebtngroup">
                          <!--<span>Minimum Bid: &#8377; <span class="min-bid">1000</span></span><br>-->
                          <span>Minimum Bid: &#8377; <span class="min-bid{{ $order->id }}">1000</span></span><br>
                              ...
                              <tbody>
                                <tr>
                                   @foreach($service_details as $service_dt)
                                    ...
                                    @endforeach
                                  <td class="nowrap">
                                      <input type="text" class="from-control provider_price" name="provider_price"  id="provider_price" Placeholder="Enter Bid Price">
                                  ...
                                  <td class="nowrap">
                                      <div class="bidbtngroup"></div><a class="bidbtn bidsubmit" href="javascript:void(0);" data-price="{{ $order->total_amount }}" data-orderid="{{ $order->id }}">Bid Now</a></div>
                                  </td>
 ...
 /* $("#submit-min-bid").click( function(){
  ...
  $(".bidsubmit").click( function(){
     ...
     var orderid = $(this).data('orderid');
      ...
      var provider_price = $(this).parent().parent().find('td').eq(0).find('.provider_price').val();
      
     // alert('price: '+price+' enter Price'+provider_price)
      
      if(parseInt(price) < provider_price){
         
          $('.price_error').text('Bid Price Not Allowed more Then Order Price.');
      ...
      if(provider_price==''){
          ...
          $('.price_error').text('Please Enter Bid Amount.');
     ...
     //console.log($(this).parent().parent().find('td').eq(0).find('.provider_price').val());
      
      //return false;
      
       $.ajax({
          type: "POST",
          url: '<?php echo url(''); ?>/user/offer-bid',
          data: {
            _token:'{{csrf_token()}}',
            orderid:orderid,
          ...
          success: function(response)
          {
                 if(response==1){
                     ...
                     $('#bidsuccess'+orderid).text('Bid Offer Successfully Send');
